using Domain.Core;
using Domain.Passengers.DataAccess;
using Domain.Passengers.Models;
using Domain.Services.Requests;
using Microsoft.EntityFrameworkCore;

namespace Infrastructure;

public class PassengerNoSpecificationProvider(AirContext airContext) : IPassengerNoSpecificationProvider
{
    private readonly AirContext airContext = airContext ?? throw new ArgumentNullException(nameof(airContext));

    /// <summary>
    /// Получаем данные о номерах рейсов пассажиров.
    /// </summary>
    /// <param name="request"></param>
    /// <returns></returns>
    public async Task<IReadOnlyCollection<PassengerFlightNumModel>> GetPessengersFlightNoAsync(GetPassengerRequest request)
    {
        // Запрос, который формирует EF.

        //SELECT p.passenger_id AS "Id", p.first_name AS "FirstName", p.last_name AS "LastName", f.flight_no AS "FlightNum"
        // FROM postgres_air.passenger AS p
        // LEFT JOIN postgres_air.booking AS b ON p.booking_id = b.booking_id
        // LEFT JOIN postgres_air.flight AS f ON(
        //     SELECT b0.flight_id
        //     FROM postgres_air.booking_leg AS b0
        //     WHERE b0.booking_id = b.booking_id
        //     LIMIT 1) = f.flight_id
        // WHERE p.passenger_id = ANY(@__request_PassengerIds_0)
        // AND NOT(p.passenger_id = ANY(@__request_PassengerIdsToExclude_1)
        // AND p.passenger_id = ANY(@__request_PassengerIdsToExclude_1) IS NOT NULL)

        //"QUERY PLAN"
        //"Hash Left Join  (cost=26608.37..36174.63 rows=5 width=20)"
        //"  Hash Cond: ((SubPlan 1) = f.flight_id)"
        //"  ->  Nested Loop Left Join  (cost=0.87..64.54 rows=5 width=24)"
        //"        ->  Index Scan using passenger_pkey on passenger p  (cost=0.43..42.29 rows=5 width=20)"
        //"              Index Cond: (passenger_id = ANY ('{1,2,3,4,5}'::integer[]))"
        //"              Filter: ((passenger_id <> ALL ('{2,3}'::integer[])) OR ((passenger_id = ANY ('{2,3}'::integer[])) IS NULL))"
        //"        ->  Index Only Scan using booking_pkey on booking b  (cost=0.43..4.45 rows=1 width=8)"
        //"              Index Cond: (booking_id = p.booking_id)"
        //"  ->  Hash  (cost=15398.78..15398.78 rows=683178 width=8)"
        //"        ->  Seq Scan on flight f  (cost=0.00..15398.78 rows=683178 width=8)"
        //"  SubPlan 1"
        //"    ->  Limit  (cost=0.00..27326.28 rows=1 width=4)"
        //"          ->  Seq Scan on booking_leg b0  (cost=0.00..355241.70 rows=13 width=4)"
        //"                Filter: (booking_id = b.booking_id)"

        var query = from ps in airContext.Passengers
                    
                    join bkngs in airContext.Bookings
                    on ps.BookingId equals bkngs.Id into bookings
                    from bk in bookings.DefaultIfEmpty()
                      
                    let bl = airContext.BookingLegals
                        .Where(x => x.BookingId.Equals(bk.Id))
                        .FirstOrDefault()
                      
                    join flts in airContext.Flights
                      on bl.FlightId equals flts.Id into flights
                      from fl in flights.DefaultIfEmpty()

                      where request.PassengerIds.Contains(ps.Id)

                      select new
                      { 
                          ps.Id,
                          ps.FirstName, 
                          ps.LastName,
                          FlightNum = fl.FlightNo,
                          ps.UpdateTs
                      };

        if (request.PassengerIdsToExclude != null)
        {
            query = query.Where( x => !request.PassengerIdsToExclude.Contains(x.Id));
        }

        if (request.UpdateTsStart.HasValue || request.UpdateTsEnd.HasValue)
        {
            if (request.UpdateTsStart.HasValue && !request.UpdateTsEnd.HasValue)
            {
                var dateFrom = GetStartDate(request.UpdateTsStart.Value);

                query = query.Where(x => x.UpdateTs >= dateFrom || x.UpdateTs == null);
            }

            if (request.UpdateTsEnd.HasValue && !request.UpdateTsStart.HasValue)
            {
                var dateTo = GetEndDate(request.UpdateTsEnd.Value);

                query = query.Where(x => x.UpdateTs < dateTo || x.UpdateTs == null);
            }

            if (request.UpdateTsEnd.HasValue && request.UpdateTsStart.HasValue)
            {
                var dateFrom = GetStartDate(request.UpdateTsStart.Value);
                var dateTo = GetEndDate(request.UpdateTsEnd.Value);

                query = query.Where( x => (x.UpdateTs >= dateFrom && x.UpdateTs < dateTo) || x.UpdateTs == null);
            }
        }

        return await query.Select(x => new PassengerFlightNumModel
            { 
                Id = x.Id,
                FirstName = x.FirstName,
                LastName = x.LastName,
                FlightNum = x.FlightNum,
            }).ToListAsync();
    }

    /// <summary>
    /// Получаем данные о номерах телефонов пассажиров.
    /// </summary>
    /// <param name="request"></param>
    /// <returns></returns>
    public async Task<IReadOnlyCollection<PassengerPhoneModel>> GetPessengersPhoneAsync(GetPassengerRequest request)
    {
        // Запрос, который формирует EF.

        //SELECT p.passenger_id AS "Id", p.first_name AS "FirstName", p.last_name AS "LastName", (
        //                SELECT p0.phone
        //                FROM postgres_air.phone AS p0
        //                WHERE p0.account_id = a.account_id
        //    LIMIT 1) AS "PhoneNumber"
        //FROM postgres_air.passenger AS p
        //LEFT JOIN postgres_air.account AS a ON p.account_id = a.account_id
        //WHERE p.passenger_id = ANY(@__request_PassengerIds_0) AND NOT(p.passenger_id = ANY(@__request_PassengerIdsToExclude_1) AND p.passenger_id = ANY(@__request_PassengerIdsToExclude_1) IS NOT NULL)

        //"QUERY PLAN"
        //"Nested Loop Left Join  (cost=0.86..20962.12 rows=5 width=48)"
        //"  ->  Index Scan using passenger_pkey on passenger p  (cost=0.43..42.29 rows=5 width=20)"
        //"        Index Cond: (passenger_id = ANY ('{1,2,3,4,5}'::integer[]))"
        //"        Filter: ((passenger_id <> ALL ('{2,3}'::integer[])) OR ((passenger_id = ANY ('{2,3}'::integer[])) IS NULL))"
        //"  ->  Index Only Scan using account_pkey on account a  (cost=0.42..4.44 rows=1 width=4)"
        //"        Index Cond: (account_id = p.account_id)"
        //"  SubPlan 1"
        //"    ->  Limit  (cost=0.00..4179.52 rows=1 width=10)"
        //"          ->  Seq Scan on phone p0  (cost=0.00..8359.05 rows=2 width=10)"
        //"                Filter: (account_id = a.account_id)"

        var query = from ps in airContext.Passengers
                    
                    join accs in airContext.Accounts
                    on ps.AccountId equals accs.Id into accounts
                    from acc in accounts.DefaultIfEmpty()
                   
                    let phn = airContext.Phones
                        .Where(x => x.AccountId.Equals(acc.Id))
                        .FirstOrDefault()
                   
                    where request.PassengerIds.Contains(ps.Id)

                    select new
                    {
                        ps.Id,
                        ps.FirstName,
                        ps.LastName,
                        ps.UpdateTs,
                        Phone = phn != null 
                            ? phn.PhoneValue 
                            : null,
                    };

        if (request.PassengerIdsToExclude != null)
        {
            query = query.Where(x => !request.PassengerIdsToExclude.Contains(x.Id));
        }

        if (request.UpdateTsStart.HasValue || request.UpdateTsEnd.HasValue)
        {
            if (request.UpdateTsStart.HasValue && !request.UpdateTsEnd.HasValue)
            {
                var dateFrom = GetStartDate(request.UpdateTsStart.Value);

                query = query.Where(x => x.UpdateTs >= dateFrom || x.UpdateTs == null);
            }

            if (request.UpdateTsEnd.HasValue && !request.UpdateTsStart.HasValue)
            {
                var dateTo = GetEndDate(request.UpdateTsEnd.Value);

                query = query.Where(x => x.UpdateTs < dateTo || x.UpdateTs == null);
            }

            if (request.UpdateTsEnd.HasValue && request.UpdateTsStart.HasValue)
            {
                var dateFrom = GetStartDate(request.UpdateTsStart.Value);
                var dateTo = GetEndDate(request.UpdateTsEnd.Value);

                query = query.Where(x => (x.UpdateTs >= dateFrom && x.UpdateTs < dateTo) || x.UpdateTs == null);
            }
        }

        return await query.Select(x => new PassengerPhoneModel
        {
            Id = x.Id,
            FirstName = x.FirstName,
            LastName = x.LastName,
            PhoneNumber = x.Phone,
        }).ToListAsync();
    }

    /// <summary>
    /// Получаем данные о номерах мест пассажиров.
    /// </summary>
    /// <param name="request"></param>
    /// <returns></returns>
    public async Task<IReadOnlyCollection<PassengerSeatModel>> GetPessengersSeatsAsync(GetPassengerRequest request)
    {
        // Запрос, который формирует EF.

        // SELECT p.passenger_id AS "Id", p.first_name AS "FirstName", p.last_name AS "LastName", (
        //                 SELECT b.seat
        //                 FROM postgres_air.boarding_pass AS b
        //                 WHERE b.passenger_id = p.passenger_id
        //     LIMIT 1) AS "BoardingPassSeat"
        // FROM postgres_air.passenger AS p
        // WHERE p.passenger_id = ANY(@__request_PassengerIds_0)
        // AND NOT(p.passenger_id = ANY(@__request_PassengerIdsToExclude_1)
        // AND p.passenger_id = ANY(@__request_PassengerIdsToExclude_1) IS NOT NULL)

        //"QUERY PLAN"
        //"Index Scan using passenger_pkey on passenger p  (cost=0.43..961587.41 rows=5 width=48)"
        //"  Index Cond: (passenger_id = ANY ('{1,2,3,4,5}'::integer[]))"
        //"  Filter: ((passenger_id <> ALL ('{2,3}'::integer[])) OR ((passenger_id = ANY ('{2,3}'::integer[])) IS NULL))"
        //"  SubPlan 1"
        //"    ->  Limit  (cost=0.00..192309.02 rows=1 width=3)"
        //"          ->  Seq Scan on boarding_pass b  (cost=0.00..576927.07 rows=3 width=3)"
        //"                Filter: (passenger_id = p.passenger_id)"
        //"JIT:"
        //"  Functions: 13"
        //"  Options: Inlining true, Optimization true, Expressions true, Deforming true"

        var query = from ps in airContext.Passengers
                    let bp = airContext.BoardingPasses
                            .Where(x => x.PassengerId.Equals(ps.Id))
                            .FirstOrDefault()
                    where request.PassengerIds.Contains(ps.Id)
                    select new
                    {
                        ps.Id,
                        ps.FirstName,
                        ps.LastName,
                        ps.UpdateTs,
                        Seat = bp != null
                            ? bp.Seat
                            : null,
                    };

        if (request.PassengerIdsToExclude != null)
        {
            query = query.Where(x => !request.PassengerIdsToExclude.Contains(x.Id));
        }

        if (request.UpdateTsStart.HasValue || request.UpdateTsEnd.HasValue)
        {
            if (request.UpdateTsStart.HasValue && !request.UpdateTsEnd.HasValue)
            {
                var dateFrom = GetStartDate(request.UpdateTsStart.Value);

                query = query.Where(x => x.UpdateTs >= dateFrom || x.UpdateTs == null);
            }

            if (request.UpdateTsEnd.HasValue && !request.UpdateTsStart.HasValue)
            {
                var dateTo = GetEndDate(request.UpdateTsEnd.Value);

                query = query.Where(x => x.UpdateTs < dateTo || x.UpdateTs == null);
            }

            if (request.UpdateTsEnd.HasValue && request.UpdateTsStart.HasValue)
            {
                var dateFrom = GetStartDate(request.UpdateTsStart.Value);
                var dateTo = GetEndDate(request.UpdateTsEnd.Value);

                query = query.Where(x => (x.UpdateTs >= dateFrom && x.UpdateTs < dateTo) || x.UpdateTs == null);
            }
        }

        return await query.Select(x => new PassengerSeatModel
        {
            Id = x.Id,
            FirstName = x.FirstName,
            LastName = x.LastName,
            BoardingPassSeat = x.Seat,
        }).ToListAsync();
    }

    private static DateTime GetStartDate(DateTime requestTo) => requestTo.Date;

    private static DateTime GetEndDate(DateTime requestFrom) => requestFrom.Date.AddDays(1);
}
